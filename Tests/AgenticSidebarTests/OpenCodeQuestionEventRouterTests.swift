...
final class OpenCodeQuestionEventRouterTests: XCTestCase {
    ...
    func testGlobalEventDoesNotAttributeForeignQuestionButReleasesVerifiedChild() throws {
        var router = OpenCodeQuestionEventRouter(sessionID: "ses_owner")
        let foreign = #"data: {"type":"question.asked","properties":{"id":"que_foreign","sessionID":"ses_foreign","questions":[{"question":"Private?","header":"Private","options":[]}]}}"#
        let child = #"data: {"type":"question.asked","properties":{"id":"que_child","sessionID":"ses_child","questions":[{"question":"Choice?","header":"Choice","options":[]}]}}"#
        XCTAssertTrue(router.consume(line: foreign).isEmpty)
        XCTAssertTrue(router.consume(line: child).isEmpty)
        let parentTask = #"data: {"type":"message.part.updated","properties":{"sessionID":"ses_owner","part":{"id":"prt_task","type":"tool","tool":"task","state":{"status":"running","input":{},"metadata":{"sessionId":"ses_child"}}}}}"#
