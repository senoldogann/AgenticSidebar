...
> **Plan revision (2026-09-19, user-approved):** the scheduler's workspace port is revised to workspace-before-claim. `TaskScheduler.schedule` mints the attempt identity first, creates an owned workspace (manifest + nonce) through an extended workspace port, then claims the attempt bound to that workspace; on claim failure the created workspace is retired by exact identity before the entry is reported deferred. Rationale: the original preflight-only port could never return `.owned` for a first attempt (no manifest existed yet), so live dispatch was structurally impossible. Recovery precondition: `TaskRecovery.reconcile` runs at launch, before the scheduler processes any turn, and is never invoked concurrently with live attempts (its heal path cannot distinguish a queued-but-unclaimed attempt from a stranded one).

- [x] Step 1: RED integration test: create a task, enforce dependency, approve execution, claim one attempt in a disposable owned worktree, receive fake OpenCode events, run fake recipe evidence, require matching user acceptance, persist/reopen and confirm done once.
- [x] Step 2: RED crash test: stop after provider reports an edit but before evidence persistence; launch again and confirm blocked uncertainty, not duplicate code-writing.
- [x] Step 3: Run `swift test --filter MultiAgentCodingIntegrationTests`, verify RED, then wire composition and service only after previous phases' gates are green.
- [x] Step 4: GREEN integration and all existing session/Goal tests; run fresh `swift build --product AgenticSidebar -Xswiftc -warnings-as-errors`, `swift test -Xswiftc -warnings-as-errors`, pinned `swift-format lint -r --strict Sources Tests`, and `git diff --check` at the exact content digest.
